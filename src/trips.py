...
class SavedTrip:
    ...
    def can_create_itineraries(self) -> bool:
        """Only the organizer can edit drafts or publish itinerary versions."""
...
def save_shared_itinerary_version(
    ...
    *,
    ...
    """Reject legacy collaborator publishing; organizers save via ``save_trip``."""
...
def list_saved_trips(
    ...
    *,
    ...
    if is_configured():
        ...
        rows = [
            row
            for row in directly_visible_rows
            if str(row.get("session_id") or "") == session_id
        ]
        if auth_access_token:
            try:
                sanitized_shared_rows = rpc_authenticated(
                    "list_shared_trips",
                    {},
                    auth_access_token,
                )
            except Exception:
                # Fail closed: older deployments must apply the current schema
                # before shared trips are returned without private profiles.
                sanitized_shared_rows = []
            if isinstance(sanitized_shared_rows, list):
                owned_identities = {
                    (
                ...
                rows.extend(
                    row
                    for row in sanitized_shared_rows
                    if (
                        str(row.get("session_id") or ""),
                        str(row.get("trip_id") or ""),
                    )
            ...
            records.append(
                SavedTrip(
                    ...
                    (
                        ...
                        else str(row.get("access_role") or "viewer")
                    ),
                )
            )
        return records
